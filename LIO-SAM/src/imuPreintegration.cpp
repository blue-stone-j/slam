...
class TransformFusion : public ParamServer
{
 ...
 public:
  ...
  TransformFusion()
  {
    ...
    {
      // If you get an error "Lookup would require extrapolation into the past" while running,
      // you can try this alternative code to call the listener:
      try
      {
  ...
  {
    // static tf, 定义一个广播，相当于发布话题时定义一个发布器
    ...
    Eigen::Affine3f imuOdomAffineLast  = lidarOdomAffine * imuOdomAffineIncre; // last pose from map opt * imu incre
...
class IMUPreintegration : public ParamServer
{
 public:
  ...
  IMUPreintegration()
  {
    // 业务逻辑都在callback里面写, 两个数据是耦合关系, imu通过激光odom给出优化后的预积分预测, odom根据预测的位姿优化、融合出新的odom
  ...
  void imuHandler(const sensor_msgs::Imu::ConstPtr &imu_raw)
  {
    ...
    // 检查有没有执行过一次优化,这里需要先在odomhandler中优化一次后再进行该函数后续的工作;return if no optimization is performed
  ...
  void odometryHandler(const nav_msgs::Odometry::ConstPtr &odomMsg)
  {
    ...
    // make sure we have imu data to integrate; 两个回调函数是互有联系的, 在imu的回调里就强调要完成一次优化才往下执行
    if (imuQueOpt.empty())
    {
      return;
    }
    ...
    */
    ...
    while (!imuQueOpt.empty())
    {
      ...
      {
        // time difference of two frames; (imu time - last imu time); if not initialized, dt is set as a small value
    ...
    {
      resetParams(); // abnormal status
      return;
    }

    // imuIntegrateImu 在每次odom触发优化后立刻获取最新的bias,然后对imu测量值imuQueImu执行bias改变的状态重传播处理
  ...
  void resetOptimization()
  {
    ...
    graphFactors = newGraphFactors; // define a new varible to clear this map
  ...
  bool failureDetection(const gtsam::Vector3 &velCur, const gtsam::imuBias::ConstantBias &biasCur)
  {
    ...
    if (ba.norm() > 1.0 || bg.norm() > 1.0) // 1 m/s; 1 rad/s
    {
